 ...
 */
double swapDouble(unsigned char * dblBuf)
{
    double swappedDblResult;
    unsigned char *dst = reinterpret_cast<unsigned char *>(& swappedDblResult);
...
/**
 * Example AlphaFlash client application used to demonstrate connect, login, receive and decode messages. 
 * 
 * This example uses Asio library for networking (included in the repository).
 */
int main(int argc, char* argv[])
{
  try
  {
	...
	asio::write(socket, asio::buffer(loginString), asio::transfer_all(), comm_error);

	std::array<char, 128> loginResponsebuf; 

	// wait for login response
	size_t len = socket.read_some(asio::buffer(loginResponsebuf), comm_error);

	if (comm_error)
		throw asio::system_error(comm_error); // Some error.
    ...
    for (;;)
    {
	  ...
	  size_t header_len = asio::read(socket, asio::buffer(msgBuf, HEADER_SIZE));
	  std::cout << "Read " << header_len << " header bytes from socket." << std::endl;

      if (comm_error == asio::error::eof)
        break; // Connection closed cleanly by peer.
      else if (comm_error)
        throw asio::system_error(comm_error); // Some other error.

	  char *msgdata = msgBuf.data();
	  unsigned short int msg_length = ntohs(*(reinterpret_cast<unsigned short int *> (&msgdata[0])) );
	  std::cout << "Msg Length:" << msg_length << std::endl;
	 
	  // read payload next
	  size_t payload_len = asio::read(socket, asio::buffer(&msgBuf[HEADER_SIZE], msg_length-HEADER_SIZE));
	  ...
	  {		  
		  ...
		  switch (field_type) 
		  {
			...
			case DOUBLE_FIELD_TYPE:
				{
					double field_value = swapDouble((unsigned char *)&msgdata[value_offset]);
